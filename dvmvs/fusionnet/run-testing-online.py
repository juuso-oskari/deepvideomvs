...
def predict(evaluate):
    ...
    K = np.loadtxt(scene_folder / 'K.txt').astype(np.float32).reshape((3,3))
    ...
    dir = '/home/juuso/office'
    if os.path.exists(dir):
        shutil.rmtree(dir)
    dir = '/home/juuso/office/'
    os.makedirs(dir)
    os.makedirs(dir + "images")
    os.makedirs(dir + "depth")
    np.savetxt(dir + "K.txt", K)
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(poses))):
            ...
            # save for 3D reconstruction of the scene 
            with open(dir + "poses.txt", "ab") as f:
                np.savetxt(f, reference_pose)
            
            
            
            import pathlib
            image_id = pathlib.PurePath(image_filenames[i]).name
            print(dir + "images/" + image_id)
            cv2.imwrite(dir + "images/" + image_id, cv2.imread(image_filenames[i]))
            cv2.imwrite(dir + "depth/" + image_id, prediction)
            
            
            predictions.append(prediction)

            if True:#Config.test_visualize:
                visualize_predictions(numpy_reference_image=reference_image,
                                      ...
                                      depth_multiplier_for_visualization=5000*5)
